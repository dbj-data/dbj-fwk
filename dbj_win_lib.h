...
#define DBJ_STATUS_SUCCESS (0x00000000)
    ...
    inline osinfo get_win_version(void)
    {
        ...
        if (hMod)
        {
            get_version_fp fxPtr = (get_version_fp)::GetProcAddress(hMod, "RtlGetVersion");
            if (fxPtr != nullptr)
            {
...
#define IS_OS_INFO_EMPTY(info_) (0 == info_.size_info_ == info_.major == info_.minor)

    /*
 return true if Windows version is greater than or equal to the specified number

 if minor_ver == 0 , minor and build arguments are ignored
 ...
 */
        ...
        WORD major_ver, WORD minor_ver, WORD build_ver)
    {
        ...
        // ignore the rest if caller sent 0 as a minor ver
        if (minor_ver == 0)
        {
            return ver_info_.major == major_ver;
        }
